...
DEFAULT_SOURCE = "slack-huddle"
DEFAULT_CHANNEL = "#firefighters"
DEFAULT_SKIP_ON_OR_BEFORE = "2025-03-10"
STATE_DB_FILENAME = "slack-huddle-state.sqlite3"
...
def pdf_matches_title(filename: str, title: str | None) -> bool:
    expected = normalize_match_text(title)
    if not expected:
        return True
    stem = Path(filename).stem
    actual = normalize_match_text(stem)
    return bool(actual) and (actual in expected or expected in actual)
...
def parse_canvas_title(title: str | None) -> tuple[str | None, str | None]:
    normalized = normalize_title(title)
    if not normalized or " in " not in normalized.casefold():
        return None, None
    prefix, _, suffix = normalized.partition(" in ")
    if not prefix.casefold().startswith("huddle notes:"):
        return None, None
    raw_date = prefix.split(":", 1)[1].strip()
    match = re.fullmatch(r"(?P<month>\d{1,2})/(?P<day>\d{1,2})/(?P<year>\d{2,4})", raw_date)
    if not match:
        return None, suffix.strip() or None
    ...
    if year < 100:
        year += 2000
    try:
        occurred_at = datetime(year, month, day).strftime("%Y-%m-%d")
    except ValueError:
        return None, suffix.strip() or None
    return occurred_at, suffix.strip() or None


def build_huddle_identity(
    *,
    thread_url: str | None,
    canvas_url: str | None,
    title: str | None,
    ...
    canonical_url = normalize_url(thread_url) or normalize_url(canvas_url)
    if canonical_url:
...
def state_db_path(staging_dir: Path) -> Path:
    return staging_dir / STATE_DB_FILENAME
...
def connect_db(staging_dir: Path) -> sqlite3.Connection:
    ...
    connection.executescript(
        ...
        CREATE TABLE IF NOT EXISTS huddles (
            ...
            channel TEXT NOT NULL DEFAULT '#firefighters',
        ...
        CREATE TABLE IF NOT EXISTS files (
            ...
            FOREIGN KEY (meeting_key) REFERENCES huddles(meeting_key) ON DELETE CASCADE
        );
...
def upsert_huddle(
    ...
    *,
    ...
    canvas_value = normalize_url(canvas_url) or None
    thread_value = normalize_url(thread_url) or None
    participants_json = json.dumps(participants)
    existing = connection.execute("SELECT * FROM huddles WHERE meeting_key = ?", (meeting_key,)).fetchone()
    if existing is None:
        connection.execute(
            """
            INSERT INTO huddles (
                meeting_key, canonical_url, canvas_url, thread_url, title, occurred_at, participants_json,
                channel, first_seen_at, last_attempt_at, state, error_message
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                meeting_key,
        ...
        merged_canvas = canvas_value or existing["canvas_url"]
        merged_thread = thread_value or existing["thread_url"]
    ...
    connection.commit()
    row = connection.execute("SELECT * FROM huddles WHERE meeting_key = ?", (meeting_key,)).fetchone()
    return row_to_dict(row) or {}


def mark_huddle_processed(
    connection: sqlite3.Connection,
    meeting_key: str,
    *,
    state: str,
...
def mark_huddle_as_run(connection: sqlite3.Connection, *, meeting_key: str, reason: str) -> dict[str, Any]:
    return mark_huddle_processed(
        connection,
        meeting_key,
        state="processed",
        transcript_id=None,
...
def upsert_file(
    ...
    *,
    ...
    connection.execute(
        """
        INSERT INTO files (meeting_key, file_type, original_filename, staged_path, downloaded_at)
        VALUES (?, ?, ?, ?, ?)
            ...
            lines.append(
                " | ".join(
                    [
                        ...
                        item.get("title") or "-",
                        item.get("organizer") or f"Slack {DEFAULT_CHANNEL}",
...
def select_canonical_file(
    ...
    *,
    ...
    if not pdfs:
        return None, []
    matching_pdfs = [path for path in pdfs if pdf_matches_title(path.name, expected_title)]
    candidate_pdfs = matching_pdfs or pdfs
    selected = latest_file_by_extension(candidate_pdfs)
    skipped: list[tuple[Path, str]] = []
    for path in pdfs:
        if path == selected:
            continue
        if path in candidate_pdfs:
            skipped.append((path, f"newer {selected.name} was chosen for ingest"))
        else:
            skipped.append((path, f"{path.name} did not match expected huddle title"))
...
def build_ingest_command(
    *,
    ...
    if normalized_source_url:
        command.extend(["--source-url", normalized_source_url])
    if participants:
...
def build_reindex_command(*, workspace: Path) -> list[str]:
    return ["uv", "run", "codex", "--workspace", str(workspace), "transcripts", "reindex", "--pending"]


def build_module_reindex_command(*, workspace: Path) -> list[str]:
    return ["uv", "run", "python", "-m", "codex.cli", "--workspace", str(workspace), "transcripts", "reindex", "--pending"]
...
def command_start_run(args: argparse.Namespace) -> int:
    ...
    started_at = utc_now()
    connection.execute("INSERT INTO runs(run_id, started_at, finished_at, summary_path) VALUES (?, ?, NULL, NULL)", (run_id, started_at))
    connection.commit()
    manifest = {
        "run_id": run_id,
...
def command_should_process(args: argparse.Namespace) -> int:
    ...
    occurred_at = normalize_occurred_at(args.occurred_at)
    if not occurred_at and args.title:
        occurred_at, _ = parse_canvas_title(args.title)
        occurred_at = occurred_at or ""
    canonical_url, meeting_key = build_huddle_identity(
        thread_url=args.thread_url,
        canvas_url=args.canvas_url,
        title=args.title,
        occurred_at=occurred_at,
    )
    huddle = upsert_huddle(
        connection,
    ...
    mark_reason = None
    if cutoff_applies(huddle.get("occurred_at"), args.skip_on_or_before):
        mark_reason = (
            f"Marked as already run because occurred_at {huddle.get('occurred_at')} "
            f"is on or before cutoff {normalize_occurred_at(args.skip_on_or_before)}."
        )
        huddle = mark_huddle_as_run(connection, meeting_key=meeting_key, reason=mark_reason)
    should_process = huddle["state"] != "processed"
    update_run_manifest(
        staging_dir,
        args.run_id,
        meeting_key,
        ...
        channel=DEFAULT_CHANNEL,
        canonical_url=canonical_url,
        canvas_url=huddle.get("canvas_url"),
        thread_url=huddle.get("thread_url"),
        title=huddle.get("title"),
        occurred_at=huddle.get("occurred_at"),
        participants=huddle.get("participants", []),
        organizer=f"Slack {DEFAULT_CHANNEL}",
...
def command_apply_cutoff(args: argparse.Namespace) -> int:
    ...
    for row in rows:
        huddle = row_to_dict(row) or {}
        meeting_key = str(huddle.get("meeting_key") or "")
        if not meeting_key:
            continue
        reason = f"Marked as already run because occurred_at {huddle.get('occurred_at')} is on or before cutoff {cutoff}."
        updated = mark_huddle_as_run(connection, meeting_key=meeting_key, reason=reason)
        update_run_manifest(
            staging_dir,
            args.run_id,
            meeting_key,
            ...
            canvas_url=updated.get("canvas_url"),
    ...
    compact_json({"cutoff": cutoff, "marked_count": len(marked_rows), "marked_meetings": marked_rows})
    return 0
...
def command_claim_downloads(args: argparse.Namespace) -> int:
    ...
    for path in downloads_dir.iterdir():
        ...
        if path.suffix.casefold() in TEMP_DOWNLOAD_SUFFIXES:
            continue
        if path.suffix.casefold().lstrip(".") != "pdf":
            continue
        if path.stat().st_mtime < args.since_epoch:
            continue
        if args.title and not pdf_matches_title(path.name, args.title):
            ignored.append(f"{path.name} did not match expected huddle title")
            continue
    ...
    for candidate in candidates:
        ...
        moved.append({"filename": candidate.name, "file_type": "pdf", "staged_path": str(destination)})
    missing = [] if moved else ["pdf"]
    meeting_state = "downloaded" if moved else "downloads_missing"
    blocked_reason = None if moved else "transcript export unavailable or blocked (pdf missing)"
    blocked_kind = None if moved else "missing_downloads"
    blocked_at = utc_now() if not moved else None
    huddle = upsert_huddle(
        connection,
        canonical_url=canonical_url,
        meeting_key=meeting_key,
...
def command_ingest_meeting(args: argparse.Namespace) -> int:
    ...
    selected_file, skipped = select_canonical_file(staged_files, expected_title=huddle.get("title"))
    if selected_file is None:
        error_message = "No staged .pdf files found for huddle."
        mark_huddle_processed(
            connection,
            meeting_key,
            state="ingest_failed",
            transcript_id=None,
        ...
        update_run_manifest(staging_dir, args.run_id, meeting_key, state="ingest_failed", error=error_message)
        compact_json({"meeting_key": meeting_key, "error": error_message})
        return 1

    for skipped_path, reason in skipped:
        update_file_decision(connection, meeting_key, str(skipped_path), selected_for_ingest=False, skip_reason=reason)
    ...
    ingest_command = build_ingest_command(
        ...
        participants=huddle.get("participants", []),
        tags=tags,
        source_url=huddle.get("thread_url"),
    )
    fallback_command = build_module_ingest_command(
        workspace=workspace,
        transcript_path=selected_file,
        title=huddle.get("title"),
        occurred_at=huddle.get("occurred_at"),
    ...
    completed, executed_command = run_codex_command(ingest_command, fallback_command)
    ...
    if completed.returncode != 0:
        ...
        update_file_decision(connection, meeting_key, str(selected_file), selected_for_ingest=True, skip_reason=None)
        update_run_manifest(
            staging_dir,
            args.run_id,
            meeting_key,
        ...
        compact_json({"meeting_key": meeting_key, "file_path": str(selected_file), "error": error_message, "stdout": stdout, "stderr": stderr})
        return 1
    ...
    try:
        ...
        compact_json({"meeting_key": meeting_key, "file_path": str(selected_file), "error": error_message, "stdout": stdout})
        return 1
    ...
    compact_json({"meeting_key": meeting_key, "selected_file": str(selected_file), "ingest_result": ingest_payload, "skipped_files": skipped_rows})
...
def command_finalize_run(args: argparse.Namespace) -> int:
            ...
            summary_rows.append(
                {
                    ...
                    "source_url": entry.get("thread_url") or entry.get("source_url"),
                }
            )
            ...
            blocked_rows.append(
                {
                    ...
                    "organizer": entry.get("organizer") or f"Slack {DEFAULT_CHANNEL}",
    ...
    finished_at = utc_now()
    connection.execute("UPDATE runs SET finished_at = ?, summary_path = ? WHERE run_id = ?", (finished_at, str(final_summary_path), args.run_id))
    connection.commit()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Track and ingest Slack #firefighters huddle note exports.")
    ...
    apply_cutoff_parser.add_argument("--on-or-before", default=DEFAULT_SKIP_ON_OR_BEFORE)
    ...
    should_process_parser.add_argument("--canvas-url")
    should_process_parser.add_argument("--thread-url")
    ...
    should_process_parser.add_argument("--skip-on-or-before", default=DEFAULT_SKIP_ON_OR_BEFORE)
    ...
    claim_parser.add_argument("--canvas-url")
    claim_parser.add_argument("--thread-url")
    claim_parser.add_argument("--title")
    claim_parser.add_argument("--occurred-at")
    ...
    ingest_parser.add_argument("--canvas-url")
    ingest_parser.add_argument("--thread-url")
