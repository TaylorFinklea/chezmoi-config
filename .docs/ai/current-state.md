...
- Dropped the global spec-agent pack (`spec-planner`, `spec-implementer`, `spec-verifier`) shipped earlier the same day. Real-world dogfood showed the synchronous tier-delegation workflow didn't earn its overhead — implementer crashes get patched at parent-tier cost, killing the discount, and the only durable value (the spec artifact) is narrow to multi-phase work already covered by hand-written `.docs/ai/phases/<slug>-spec.md` files plus `/plan-backlog-item`. Removed the agents and slash commands across Claude Code, GitHub Copilot CLI, and OpenCode and stripped every doc reference. The AI catalog still reflects the plural agent/command roots so future global agents can be added without re-plumbing.
