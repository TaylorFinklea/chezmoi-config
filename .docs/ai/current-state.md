...
- Added two global Claude Code hooks to backstop the AGENTS.md `.docs/ai/` workflow: `dot_claude/hooks/handoff-freshness.sh` (SessionStart, synchronous) injects an `additionalContext` summary of days since `current-state.md` was updated and how many source-touching commits / uncommitted files have piled up since; `dot_claude/hooks/handoff-rewake.sh` (Stop, `asyncRewake: true`) exits 2 once per session if this session changed source files without touching `current-state.md`. Both no-op outside repos that have `.docs/ai/`. Wired into `dot_claude/settings.json.tmpl` alongside the existing moshi/bytesaddle entries; session-start SHA and one-shot fired sentinels live under `~/.cache/claude-handoff/`. Applied live.
- Wired bytesaddle into the chezmoi-managed Claude config (commit `6c01567`): `dot_claude/settings.json.tmpl` gained SessionStart and SessionEnd hook entries invoking `~/git/bytesaddle/harness/capture/hooks/session_{start,end}.py` via `uv run --directory`, leaving the existing moshi-hook entries untouched. `dot_config/opencode/opencode.json.tmpl` also picked up a personal-profile-only `litellm` provider pointing at the local LiteLLM proxy with `LITELLM_MASTER_KEY` and six exposed models (kimi-k2, claude-{sonnet,opus}, gpt-5, deepseek-v3, qwen3-coder-hosted).
- Fixed the Claude Code Notification bell: the managed Notification hook now emits `{"terminalSequence":"\u0007"}` via `jq` instead of writing BEL to `/dev/tty`. Claude Code runs hooks without a controlling terminal, so the old `printf '\a' > /dev/tty` command silently failed with `device not configured`; the `async: true` flag also had to come off since async hooks skip stdout parsing. Applied to `~/.claude/settings.json`; next session start picks it up and tmux's `monitor-bell` path lights up Ghostty's `bell-features` again.
- Removed the explicit Ghostty `clipboard-read` override so reads return to the default prompt-on-read behavior, and disabled Yanky's system-clipboard focus sync inside tmux. Neovim/tmux yanks still write to the macOS clipboard, but pane/window focus changes should no longer trigger OSC 52 clipboard-read prompts.
