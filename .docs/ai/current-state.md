...
- Disabled Moshi-backed Codex hooks for the work AI profile: work now renders `codex_hooks = false` and an empty `~/.codex/hooks.json`, while personal keeps the Moshi hook commands. Also fixed `dot_zshenv` to call `brew shellenv zsh` explicitly so zsh startup does not parse Fish syntax when `$SHELL` points at Fish.
