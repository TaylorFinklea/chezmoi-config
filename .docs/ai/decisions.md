...
2026-05-09: Manage tmux plugin repositories as chezmoi `git-repo` externals in `.chezmoiexternal.toml`, with weekly refreshes and fast-forward-only pulls. This keeps TPM, tmux-resurrect, tmux-continuum, tmux-sensible, tmux-which-key, and vim-tmux-navigator bootstrapped by `chezmoi apply` without vendoring third-party plugin trees into this dotfiles repo. Stale Home Manager tmux symlinks are removed via `.chezmoiremove`; `~/.tmux.conf` remains the canonical tmux config.

2026-05-09: Add OpenCode to the shared hosted GitHub MCP entry using `GITHUB_PAT_TOKEN` in an Authorization header and `oauth: false`, matching GitHub's documented PAT path for non-Copilot-host OAuth clients. Rename the personal hosted Supabase MCP from `supabase-personal` to `supabase`; the Codex plugin collision that originally forced the suffix is no longer worth the inconsistent server name, and OpenCode should get a fresh OAuth client identity after the rename.
...
2026-05-15: Ring the Claude Code Notification bell via the hook's `terminalSequence` JSON output (`jq -nc '{terminalSequence: "\u0007"}'`) instead of `printf '\a' > /dev/tty`. Claude Code spawns hooks without a controlling terminal, so `/dev/tty` now fails with `device not configured` and the BEL never reaches tmux/Ghostty. `async: true` also has to come off, since async hooks skip stdout parsing and would discard the `terminalSequence` field.

2026-05-19: Enforce the AGENTS.md `.docs/ai/` handoff workflow with two Claude Code hooks rather than relying on instruction-following alone. A synchronous `SessionStart` hook injects a freshness summary (days since `current-state.md` was last touched, source-touching commit count since, uncommitted file counts) as `additionalContext`, making staleness visible up front. A `Stop` hook with `asyncRewake: true` exits 2 with a system-reminder once per session when source files changed without `current-state.md` being touched — fires at most once via a sentinel under `~/.cache/claude-handoff/`, honors `stop_hook_active`, and scopes drift to commits made after the session-start SHA so it nags only about debt this session created. Both no-op in repos without `.docs/ai/`, so the convention itself is the opt-in. Chose this over (a) visibility-only and (b) tightening instructions further because real-world observation showed the workflow was being skipped despite being clearly documented; multiple recent commits shipped without doc updates and partial doc updates were left uncommitted.
